...
const CategoriesListPage: React.FC = () => {
    ...
    const deleteCategory = async () => {
        ...
        const toastId = toast.loading("Deleting category..."); // Запуск прогрес-бару
        try {
            ...
            toast.update(toastId, { render: "Failed to delete category", type: "error", isLoading: false, autoClose: 5000 }); // Повідомлення про помилку
    ...
    return (
        <>
            <button onClick={()=>navigate('/categories/create-edit/0')} type="button" className="fixed top-20 right-10 px-3 py-2 text-xl font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            ...
                            <tr key={item.id}>
                                ...
                                <td className={classes}>
                                    <div className=' flex gap-4 justify-end mr-24'>
                                        ...
                                        <ToastContainer />
                                        <div key={item.id}>
                                            <button type="button" onClick={() => openModal(item.id)} className="bg-red-400 hover:bg-red-600 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
